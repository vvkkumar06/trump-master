...
const GameView = ({ route, navigation }) => {
  ...
  const [lostPlayerCards, setLostPlayerCards] = useState(undefined);
  const [shuffleData, setShuffleData] = useState(undefined);
  ...
  useEffect(() => {
    ...
    socket.on('game-over', ({ gameState, winner, postGameData }) => {
      winnerFontSize.setValue(0);
      setGameState(gameState);
      setWinner(winner);
      setShuffleData(postGameData.shuffleData);
      postRoundAnimation(() => {
        postGameData && setLostPlayerCards(postGameData.lostPlayerCards);
      });
    });
